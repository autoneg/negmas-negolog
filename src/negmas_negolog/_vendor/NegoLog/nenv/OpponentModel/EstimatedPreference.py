...
class EstimatedPreference(Preference):
    ...
    def __init__(self, reference: Preference):
        ...
        """
        # Handle adapters that don't have a JSON file path
        ...
        if reference.profile_json_path is None:
            ...
            self._issue_weights = {}
            self._value_weights = {}
            self._reservation_value = reference.reservation_value

            for issue in self._issues:
                self._issue_weights[issue] = 1.0 - reference.issue_weights[issue]
                self._value_weights[issue] = {}
                for value in issue.values:
                    self._value_weights[issue][value] = (
                        1.0 - reference.value_weights[issue][value]
                    )
        else:
            super(EstimatedPreference, self).__init__(
                reference.profile_json_path, generate_bids=False
            )
    ...
    def normalize(self):
        ...
        * Sum of Issue weights must be **1.0**
        ...
        for issue in self.issues:
            if issue_total == 0:
                self._issue_weights[issue] = 1.0 / len(self.issues)
            else:
            ...
            for value in issue.values:
                if max_val == 0:
                    self._value_weights[issue][value] = 1.0
                else:
