...
class HardHeaded(nenv.AbstractAgent):
    ...
    **HardHeaded agent by Thijs van Krimpen**:
    ...
    oppUtility: Optional[
        nenv.OpponentModel.EstimatedPreference
    ]  #: Estimated preferences of the opponent
    numberOfIssues: int  #: Number of issues in that domain

    valueCounter: Dict[
        nenv.Issue, Dict[str, float]
    ]  #: Value counter for Frequentist approach
    ...
    def initiate(self, opponent_name: Optional[str]):
        ...
        for issue in self.oppUtility.issues:
            ...
            for value in issue.values:
                self.oppUtility[issue, value] = 1.0
                self.valueCounter[issue][value] = 1.0

        # Update minimum bid utility depending on the reservation value
        self.MINIMUM_BID_UTILITY = max(
            self.MINIMUM_BID_UTILITY, self.preference.reservation_value
        )
    ...
    def receive_offer(self, bid: nenv.Bid, t: float):
        ...
        elif self.preference.get_utility(bid) > self.preference.get_utility(
            self.opponentbestbid
        ):
            self.opponentbestbid = bid.copy()

        # Update opponent best entry
        opbestvalue = self.floorEntry(
            self.BSelector.BidList, self.preference.get_utility(self.opponentbestbid)
        )[0]
        ...
        while (
            self.floorEntry(self.BSelector.BidList, opbestvalue)[1]
            != self.opponentbestbid
            and try_counter < 1000
        ):
    ...
    def updateLearner(self):
        ...
        maximumWeight = 1.0 - self.numberOfIssues * goldenValue / totalSum

        # re-weighting issues while making sure that the sum remains 1
        for i in lastDiffSet:
            if (
                lastDiffSet[i] == 0
                and self.oppUtility.get_issue_weight(i) < maximumWeight
            ):
        ...
        for issue in self.preference.issues:
            self.valueCounter[issue][self.opponentLastBid[issue]] = (
                self.valueCounter[issue][self.opponentLastBid[issue]]
                + self.LEARNING_VALUE_ADDITION
            )
    ...
    def act(self, t: float) -> nenv.Action:
        ...
        if self.firstRound:
            ...
            self.offerQueue = []
            newBids = {}
            newBid = self.lowerEntry(
                self.BSelector.BidList, self.bidHistory.myBids[-1][0]
            )
            newBids[newBid[0]] = newBid[1]

            if newBid[0] < p:
                indexer = self.random100.choice(
                    list(range(len(self.bidHistory.myBids)))
                )
                del newBids[newBid[0]]

                newBids[self.bidHistory.myBids[indexer][0]] = self.bidHistory.myBids[
                    indexer
                ][1]
            ...
            if len(newBids) <= self.TOP_SELECTED_BIDS:
                ...
                while addedSofar <= self.TOP_SELECTED_BIDS:
                    ...
                    for e in newBids:
                        if self.oppUtility.get_utility(
                            newBids[e]
                        ) > self.oppUtility.get_utility(bestBid[1]):
                            bestBid = (e, newBids[e])
        ...
        if len(self.offerQueue) == 0 or self.offerQueue is None:
            ...
            if (
                self.opponentLastBid is not None
                and self.can_accept()
                and self.preference.get_utility(bestBid1)
                <= self.preference.get_utility(self.opponentLastBid)
            ):
        ...
        if (
            ...
            and (
                self.preference.get_utility(self.opponentLastBid)
                > self.lowestYetUtility
                or self.preference.get_utility(self.offerQueue[0][1])
                <= self.preference.get_utility(self.opponentLastBid)
            )
        ):
            return self.accept_action
