...
class MICROAgent(nenv.AbstractAgent):
    ...
    **MiCRO agent by Dave de Jonge**:
    ...
    def act(self, t: float) -> Action:
        ...
        if (
            self.my_last_bid.utility
            <= self.last_received_bids[-1]
            >= self.preference.reservation_value
        ):
        ...
        if (
            ready_to_concede
            and self.my_last_bid.utility > self.preference.reservation_value
        ):
            ...
            # Select a random bid
            self.my_last_bid = (
                self.preference.bids[0]
                if self.index == 1
                else random.choice(self.preference.bids[: self.index])
            )
