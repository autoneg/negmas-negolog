    ...
    **PonPokoAgent by Takaki Matsune**:
    ...
    def act(self, t: float) -> Action:
        if self.pattern == 0:
            ...
            self.threshold_high = 1.0 - 0.1 * t
            self.threshold_low = 1 - 0.1 * t - 0.15 * abs(math.sin(t * 20))
        elif self.pattern == 3:
            self.threshold_high = 1.0 - 0.05 * t
            self.threshold_low = 1.0 - 0.1 * t

            if t > 0.99:
                self.threshold_low = 1 - 0.3 * t
        elif self.pattern == 4:
            self.threshold_high = 1.0 - 0.15 * t * abs(math.sin(t * 20))
            self.threshold_low = 1.0 - 0.21 * t * abs(math.sin(t * 20))
        else:
            self.threshold_high = 1.0 - 0.1 * t
            self.threshold_low = 1.0 - 0.2 * abs(math.sin(t * 40))
