...
from agents.AhBuNeAgent.linearorder.OppSimpleLinearOrdering import (
    OppSimpleLinearOrderding,
)
...
class AhBuNeAgent(nenv.AbstractAgent):
    ...
    **AhBuNeAgent by Ahmet Burak Yıldırım**:
    ...
    def initiate(self, opponent_name: Optional[str]):
        ...
        self.utilityLowerBound = 1.0
        self.ourMaxCompromise = 0.1

        self.lostElicityScore = 0.0
        ...
        self.ourLinearPartialOrdering = SimpleLinearOrdering(
            self.preference, list(reversed(self.allPossibleBids.copy()))
        )
    ...
    def makeAnOffer(self, t: float) -> Offer:
        ...
        ourOffer = self.ourSimilarityMap.findBidCompatibleWithSimilarity(
            self.utilityLowerBound, oppMaxBid
        )
        ...
        if t < 0.015:
            ...
            if self.oppLinearPartialOrdering.isAvailable():
                ...
                while (
                    count < 500
                    and not self.oppSimilarityMap.isCompromised(
                        ourOffer, self.oppNumFirstBids, 0.85
                    )
                    and self.ourLinearPartialOrdering.getMaxBid() == ourOffer
                ):
                    ourOffer = self.ourSimilarityMap.findBidCompatibleWithSimilarity(
                        0.85, oppMaxBid
                    )
                ...
                while (
                    count < 500
                    and ourOffer == self.ourLinearPartialOrdering.getMaxBid()
                ):
                    ourOffer = self.ourSimilarityMap.findBidCompatibleWithSimilarity(
                        0.85, oppMaxBid
                    )
        ...
        # If we have more than one received bid
            ...
            if self.ourSimilarityMap.isCompatibleWithSimilarity(
                self.lastReceivedBid, 0.9
            ):
                return Offer(self.lastReceivedBid)  # Offer the same
            if self.ourSimilarityMap.isCompatibleWithSimilarity(oppMaxBid, 0.9):
                return Offer(
                    oppMaxBid
                )  # Offer the maximum important value for the opponent

            # Limit
            count = 0
            while (
                count < 500
                and self.oppLinearPartialOrdering.isAvailable()
                and not self.oppSimilarityMap.isCompromised(
                    ourOffer, self.oppNumFirstBids, self.utilityLowerBound
                )
            ):
                ourOffer = self.ourSimilarityMap.findBidCompatibleWithSimilarity(
                    self.utilityLowerBound, oppMaxBid
    ...
    def doWeAccept(self, bid: nenv.Bid, t: float) -> bool:
        ...
        if self.oppLinearPartialOrdering.isAvailable():
            for i in range(int(startUtilitySearch * 100), 96, 5):
                ...
                if self.oppSimilarityMap.isCompromised(
                    bid, self.oppNumFirstBids, utilityTest
                ):
    ...
    def strategySelection(self, t: float):
        ...
        self.ourNumFirstBids = self.getNumFirst(
            self.utilityLowerBound, self.ourKnownBidNum
        )
        self.ourNumLastBids = self.getNumLast(
            self.utilityLowerBound,
            self.getUtilityLowerBound(1.0, self.lostElicityScore),
            self.ourKnownBidNum,
        )

        self.ourSimilarityMap.createConditionLists(
            self.ourNumFirstBids, self.ourNumLastBids
        )
        ...
        if self.lastReceivedBid is not None:
            ...
            self.oppNumFirstBids = self.getOppNumFist(
                self.utilityLowerBound, self.oppKnownBidNum
            )
    ...
    def getNumLast(
        self, utilityLowerBound: float, minUtilityLowerBound: float, knownBidNum: int
    ) -> int:
        """
        ...
        """
        return int(knownBidNum * (1 - minUtilityLowerBound)) - int(
            knownBidNum * (1 - utilityLowerBound) + 1
        )
